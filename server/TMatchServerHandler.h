...
const string DEF_IMG_PATH = "/data/demo/data/";
...
public:
    ...
    void tiqutezheng(int i)
    {
        ...
        //加载kmeans vocabulary
        oImgCtl.LoadVocabulary("/data/demo/kmeans.vob", DEF_KMEANS_K);
        ...
        for(uint32_t uiCur = 0; uiCur < DEF_MAX_IMG; uiCur++)
        {
            ...
            snprintf(szTmp, sizeof(szTmp), "/data/demo/bow/%u", uiCur);
    ...
    TMatchServerHandler() {
        ...
        for (uint32_t uiCur = 0; uiCur < DEF_MAX_IMG; uiCur++)
        {
            snprintf(szBuf, sizeof(szBuf), "/data/demo/bow/%u", uiCur);
            fp.open(szBuf);
            memset(arImgBow[uiCur], 0, DEF_KMEANS_K);
